/**
 * ============================================================
 *  NewsEditor — Production-Ready Modular JavaScript (adapted from blog.js)
...
class SlugManager {
  ...
  _regen() {
    ...
    $(document).trigger('news:slugChanged');
  }
  ...
  _copy() {
    ...
    Utils.copyToClipboard(url)
      .then(() => {
        ...
        $(document).trigger('news:toast', ['URL copied to clipboard', 'var(--success)', 'fas fa-check-circle']);
      })
      .catch(() => {
        $(document).trigger('news:toast', ['Could not copy — please copy manually', 'var(--warning)', 'fas fa-exclamation-triangle']);
      });
  }
}
...
class SerpPreview {
  ...
  _bind() {
    ...
    $('#newsTitle, #metaTitle, #newsExcerpt, #metaDescription').on('input', refresh);
    $(document).on('news:slugChanged', refresh);
  }

  refresh() {
    const slug  = this.slugManager.getSlug() || 'your-post-slug';
    const title = $('#metaTitle').val() || $('#newsTitle').val() || 'Your post title will appear here…';
    const desc  = $('#metaDescription').val() || $('#newsExcerpt').val() ||
                  'Your meta description will appear here. Write a compelling summary to improve click-through rate.';

    this.$url.text(`${this.domain} › newsroom › ${slug}`);
...
class SocialPreview {
  ...
  _bind() {
    ...
    $('#ogTitle, #ogDescription, #newsTitle, #metaTitle, #metaDescription, #newsExcerpt').on('input', refresh);
  }

  refresh() {
    const tab   = $('.sp-tab.active').data('tab') || 'facebook';
    const title = $('#ogTitle').val() || $('#metaTitle').val() || $('#newsTitle').val() || 'Your post title…';
    const desc  = $('#ogDescription').val() || $('#metaDescription').val() || $('#newsExcerpt').val() || 'Description appears here.';
...
class ImageUploader {
  ...
  _onFileSelected() {
    ...
    if (file.size > 5 * 1024 * 1024) {
      $(document).trigger('news:toast', ['Image must be under 5 MB', 'var(--danger)', 'fas fa-exclamation-circle']);
      return;
    }
    ...
    reader.onload = (e) => {
      ...
      $(document).trigger('news:imageChanged');
      $(document).trigger('news:toast', ['Featured image set', 'var(--success)', 'fas fa-image']);
    };
  ...
  _remove() {
    ...
    $(document).trigger('news:imageChanged');
    $(document).trigger('news:toast', ['Featured image removed', 'var(--warning)', 'fas fa-trash']);
  }
...
class SeoScorer {
  ...
  _densityColor(d) {
    if (d >= NewsEditorConfig.kwDensityMin && d <= NewsEditorConfig.kwDensityMax)
      return { bg: '#d4f5ec', color: '#00a87c' };
    if (d > NewsEditorConfig.kwDensityMax)
  ...
  compute() {
    let score = 0;
    const C = NewsEditorConfig;

    const kw          = $('#focusKeyword').val().trim().toLowerCase();
    const title       = $('#newsTitle').val().toLowerCase();
    const slug        = $('#newsSlug').val().toLowerCase();
...
class DraftManager {
  ...
  _resolveLocalKey() {
    ...
    return this.draftId ? ('news_draft_' + this.draftId) : 'news_draft_new';
  }
  ...
  restore() {
    try {
      ...
      if (data.title)            $('#newsTitle').val(data.title).trigger('input');
      if (data.slug)             $('#newsSlug').val(data.slug);
      if (data.excerpt)          $('#newsExcerpt').val(data.excerpt);
    ...
    $.ajax({
      ...
      success: (res) => {
        if (res.draft_id && res.draft_id !== this.draftId) {
          ...
          // it isn't orphaned under the temporary 'news_draft_new' slot.
  ...
  _startTimers() {
    ...
    setInterval(() => this.saveLocal(), NewsEditorConfig.autosaveInterval);
    // Server: every 15s
    setInterval(() => this.saveServer(), NewsEditorConfig.serverSaveInterval);
    // Also save on typing
    const debouncedSave = Utils.debounce(() => this.saveLocal(), 1500);
    $('#newsTitle, #newsExcerpt, #metaTitle, #metaDescription, #focusKeyword, #imageAlt').on('input', debouncedSave);
  }
...
class FormSubmitter {
  ...
  _bind() {
    ...
    $(document).on('click', '.js-news-save-btn', (e) => this.handleSaveClick(e));

    $('#btnPreview').on('click', () => {
      const slug = this.slugManager.getSlug();
      if (slug) {
        window.open(`/newsroom/${slug}?preview=1`, '_blank');
      } else {
        $(document).trigger('news:toast', ['Set a slug before previewing', 'var(--warning)', 'fas fa-exclamation-triangle']);
      }
    });
    ...
    $('#btnSaveCat').on('click', () => {
      ...
      if (!name) return;
      // Persist it server-side (POST /news/category/store) instead of just
      ...
      $.ajax({
        ...
        success: (res) => {
          ...
          $(document).trigger('news:toast', [`Category "${res.name}" added`, 'var(--success)', 'fas fa-folder-plus']);
        },
        error: (xhr) => {
          const msg = (xhr.responseJSON && xhr.responseJSON.message) || 'Could not add category';
          $(document).trigger('news:toast', [msg, 'var(--danger)', 'fas fa-exclamation-circle']);
        },
      });
    });
  }
  ...
  handleSaveClick(e) {
    ...
    $('#newsContent').val(this.quill.root.innerHTML);
    ...
    if (!$('#newsTitle').val().trim()) {
      e.preventDefault();
      $(document).trigger('news:toast', ['Please enter a post title', 'var(--danger)', 'fas fa-exclamation-circle']);
      $('#newsTitle').focus();
      return;
    }
    if (!$('#newsContent').val().trim() || this.quill.getText().trim().length < 10) {
      e.preventDefault();
      $(document).trigger('news:toast', ['Content is too short to save', 'var(--danger)', 'fas fa-exclamation-circle']);
      return;
    }
    if (!$('#newsSlug').val().trim()) {
      this.slugManager.updateFromTitle($('#newsTitle').val());
    }
...
class ContentStats {
  ...
  update(html) {
    ...
    const words   = Utils.countWords(text);
    const readMin = Math.max(1, Math.ceil(words / NewsEditorConfig.wordsPerMinute));
...
class ToastManager {
  constructor(stackId) {
    ...
    $(document).on('news:toast', (e, msg, color, icon) => this.show(msg, color, icon));
...
class NewsEditor {
  ...
  constructor(opts) {
    ...
    this.titleCounter    = new CharCounter('#newsTitle',        '#titleCounter',     NewsEditorConfig.maxTitleLength,   NewsEditorConfig.titleIdealMin,  NewsEditorConfig.titleIdealMax);
    this.excerptCounter  = new CharCounter('#newsExcerpt',      '#excerptCounter',   NewsEditorConfig.maxExcerptLength, 120, 160);
    this.metaTitleCount  = new CharCounter('#metaTitle',        '#metaTitleCounter', NewsEditorConfig.maxMetaTitleLen,  NewsEditorConfig.metaTitleIdealMin, NewsEditorConfig.maxMetaTitleLen, 'metaTitleMeter');
    this.metaDescCount   = new CharCounter('#metaDescription',  '#metaDescCounter',  NewsEditorConfig.maxMetaDescLen,   NewsEditorConfig.metaDescIdealMin,  NewsEditorConfig.maxMetaDescLen,  'metaDescMeter');
  ...
  _bindTitle() {
    ...
    $('#focusKeyword, #metaTitle, #metaDescription, #imageAlt, #newsExcerpt').on('input', () => this.seoScorer.compute());
    $(document).on('news:imageChanged', () => this.seoScorer.compute());
  }
